...
export function AccountSettings() {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            if (!error && data) {
                setProfile({ staffId: data.StaffID || '', username: data.Username || '', email: data.Email || '', nickname: data.Nickname || '' });
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            ...
            <div className="space-y-2 mt-6">
                ...
                <div className="border border-gray-200 rounded-xl bg-white overflow-hidden transition-all shadow-sm">
                    <button 
                        onClick={() => setExpandedOption(e => e === 'nickname' ? null : 'nickname')}
                        className="w-full flex items-center p-4 hover:bg-gray-50 text-left transition-colors"
                    >
                        <div className="w-10 h-10 flex items-center justify-center mr-4 shrink-0">
                            <User className="w-6 h-6 text-gray-700" />
                        ...
                        <div className="flex-1">
                            <p className="font-semibold text-gray-900">Nickname / Nama Panggilan</p>
                        </div>
                        <ChevronDown className={`w-5 h-5 text-gray-400 transition-transform duration-200 pr-4 box-content ${expandedOption === 'nickname' ? 'rotate-180' : ''}`} />
                    </button>
                    
                    {expandedOption === 'nickname' && (
                        <div className="px-8 py-6 bg-gray-50 border-t border-gray-100">
                            <div className="w-full max-w-2xl mx-auto space-y-4 pt-2">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                ...
                                <div className="pt-4 flex justify-end gap-3 border-t border-gray-200 mt-6">
                                    <Button 
                                        ...
                                        onClick={() => {
                                            setExpandedOption(null); setNicknameStatus('');
                                        }}
                                    >
                                        Cancel
                                    </Button>
                                    <Button onClick={handleUpdateNickname} disabled={nicknameStatus === 'loading'} className="bg-indigo-600 hover:bg-indigo-700 px-6 min-w-[120px]">
                                        {nicknameStatus === 'loading' ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : 'Update'}
                                    </Button>
                                </div>
                                
                                <div className="flex justify-end pt-1">
                                    {nicknameStatus === 'error' && <p className="text-sm font-medium text-red-600">Gagal update nickname.</p>}
                                    {nicknameStatus === 'success' && <p className="text-sm font-medium text-green-600">Nickname berjaya diupdate!</p>}
                                </div>
                            </div>
                        </div>
                    )}
