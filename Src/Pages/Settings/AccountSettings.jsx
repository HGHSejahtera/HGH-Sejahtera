        ...
        {Loading ? <p role="status" className="p-6 rounded-2xl bg-white border">{Text('Loading')}</p> : LoadError || !Profile ? <div className="p-6 rounded-2xl bg-white border space-y-3"><p role="alert">{Text('LoadFailed')}</p><Button onClick={() => { SetLoading(true); SetLoadError(false); SetRevision(Previous => Previous + 1); }}>{Text('Retry')}</Button></div> : <>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                ...
                <AccountSection Title={Text('SignInDetails')} Icon={Lock}>
                    ...
                    <AccountRow Label={Text('Password')}><Button variant="ghost" className="min-h-11 text-primary" disabled={!AccountChangesReady || Busy || PasswordChanged} onClick={()=>Open('Password')} aria-label={Text('ChangePassword')}>{Text('Change')}</Button></AccountRow>
                </AccountSection>
                <AccountSection Title={Text('ScreenLock')} Icon={Shield}>
                    <AccountRow Label={Text('PIN')} Value={Text(Auth.user?.hasPin ? 'PINActive' : 'PINOff')}><div className="flex flex-wrap gap-2">{Auth.user?.hasPin && <Button variant="ghost" className="min-h-11 text-red-700" disabled={!PINChangesReady || Busy || PasswordChanged} onClick={()=>Open('DisablePIN')} aria-label={Text('DisablePIN')}>{Text('Disable')}</Button>}<Button variant="ghost" className="min-h-11 text-primary" disabled={!PINChangesReady || Busy || PasswordChanged} onClick={()=>Open('SetPIN')} aria-label={Text(Auth.user?.hasPin ? 'ChangePIN' : 'SetPIN')}>{Text(Auth.user?.hasPin ? 'Change' : 'SetPIN')}</Button></div></AccountRow>
