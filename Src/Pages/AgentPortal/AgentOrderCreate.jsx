...
import { FetchFileAPI } from '@/Lib/FileAccess';

async function fetchWithRetry(url, options, maxRetries = 3, delayMs = 1000) {
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const response = url === '/api/generate-r2-url'
                ? await FetchFileAPI(url, options)
            ...
            if (response.status >= 500 && attempt < maxRetries) {
                console.warn(`[Retry ${attempt}/${maxRetries}] Upload returned ${response.status}. Retrying in ${delayMs}ms...`);
            ...
            if (attempt < maxRetries) {
                console.warn(`[Retry ${attempt}/${maxRetries}] Upload request failed. Retrying in ${delayMs}ms...`);
                await new Promise(r => setTimeout(r, delayMs * attempt));
            }
        }
    }
