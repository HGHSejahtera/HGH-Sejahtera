...
export function ImageDropzone({ value, onChange, className }) {
    ...
    const onDrop = useCallback(async (acceptedFiles) => {
        ...
        try {
            ...
            if (compressedFile.type !== 'image/webp') throw new Error('Unable to convert this image to WebP.');
            const res = await FetchFileAPI('/api/generate-r2-url', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ fileName, fileType: 'image/webp', FileSize: compressedFile.size })
            });
